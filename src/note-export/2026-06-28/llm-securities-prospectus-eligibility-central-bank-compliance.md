...
#ビジネス #AI活用 #金融 #コンプライアンス #LLM #RegTech
...
抽出された情報を標準的なフォーマットに変換します。「2026年1月満期」「Jan. 2026 maturity」「26/01満期」といった表記ゆれを統一するイメージです。OCRノイズの補正もここで対応します。
...
従来のNERは「正解のラベルと一致するか」で精度を測れます。しかし生成型の出力は自然言語なので、「2026年1月満期」と「Jan. 2026 maturity」は意味的に同じでも文字列一致ではありません。
